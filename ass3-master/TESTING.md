...
Since we really cannot open cypress in our local environment, we cannot test the ui_testing.js file in cypress. But we still write some code for Happy Path and the second path to add a question in a game. The code is shown below, and there also have a file named `ui_testing.js` contains the same code.
